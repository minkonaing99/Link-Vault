...
function pickBestTitle(rawTitle, ogTitle) {
  ...
  // Prefer it unless it's suspiciously short (< 5 chars), which likely means
  // a placeholder or broken tag.
...
async function fetchTitle(url, host) {
  ...
  const html = await streamHtmlHead(url);
  if (!html) return '';
  ...
  return pickBestTitle(rawTitle, ogTitle);
}
