...
async function destroySession(req, res) {
  ...
  if (token) await collections.sessions.deleteOne({ token: hashToken(token) });
...
async function getCookieSessionUser(req) {
  ...
  const session = await collections.sessions.findOne({ token: hashToken(token) });
...
async function revokeRefreshToken(token) {
  ...
  const result = await collections.refreshTokens.updateOne(
    { token: hashToken(token), revokedAt: null },
...
async function findValidRefreshToken(token) {
  await connectDb();
  if (!token) return null;
  const refreshToken = await collections.refreshTokens.findOne({ token: hashToken(token) }, { projection: { _id: 0 } });
